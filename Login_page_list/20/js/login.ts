/**
 * Login page — Design 20: Neo Brutalism · Micro-interaction · Input-reactive animation
 * Alpine.js + GSAP micro/input (compiled from TypeScript)
 */
...
function brutalMicroLogin(): Record<string, unknown> & {
  email: string;
  password: string;
...
function initInputReactive(): void {
  ...
  document.querySelectorAll("[data-brutal-field]").forEach((wrap) => {
    ...
    input.addEventListener("focus", () => {
      gsap.to(wrap, { scale: 1.02, duration: 0.2, ease: "power2.out" });
    });
