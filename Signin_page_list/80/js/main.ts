/**
 * 시안 80 - 실험적 인터페이스 + 단계 분할형(Progressive Form) + GSAP 기반 부드러운 전환
 */
declare global {
  interface Window {
...
function runPanelIn80(stepNum: number): void {
  const g = (window as unknown as { gsap?: { fromTo: (t: HTMLElement, from: object, to: object) => void } }).gsap;
  const panels = document.querySelectorAll<HTMLElement>('.s80-panel');
...
function signupForm80(): Record<string, unknown> {
  return {
    step: 1,
    email: '',
    name: '',
    ...
    goNext(): void {
      ...
      requestAnimationFrame(() => runPanelIn80(2));
    },

    goPrev(): void {
      (this as { step: number }).step = 1;
      requestAnimationFrame(() => runPanelIn80(1));
    },
...
if (typeof document !== 'undefined') {
  document.addEventListener('alpine:init', () => { if (window.Alpine) window.Alpine.data('signupForm80', signupForm80); });
