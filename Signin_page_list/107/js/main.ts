...
function signupForm107(): Record<string, unknown> {
  return {
    step: 1,
    email: '',
    name: '',
...
if (typeof document !== 'undefined') {
  document.addEventListener('alpine:init', () => { if (window.Alpine) window.Alpine.data('signupForm107', signupForm107); });
  document.addEventListener('DOMContentLoaded', () => {
    const g = (window as unknown as { gsap?: { from: (el: string | HTMLElement, v: object) => void } }).gsap;
    if (g) {
      g.from('.s107-layout', { opacity: 0, y: 8, duration: 0.38, ease: 'power2.out' });
      g.from('.s107-header', { opacity: 0, y: -4, duration: 0.3, ease: 'power2.out', delay: 0.05 });
      g.from('.s107-panels', { opacity: 0, duration: 0.28, ease: 'power2.out', delay: 0.1 });
    }
  });
}
export { signupForm107 };
