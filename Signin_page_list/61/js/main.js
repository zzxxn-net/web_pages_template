...
var FOCUS_LABELS = { email: '이메일', name: '이름', password: '비밀번호', passwordConfirm: '비밀번호 확인' };
...
if (typeof document !== 'undefined') {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    var g = window.gsap;
    if (g) {
      g.from('.s61-layout', { opacity: 0, y: 12, duration: 0.48, ease: 'power2.out' });
      g.from('.s61-aside', { x: -12, opacity: 0, duration: 0.4, delay: 0.08, ease: 'power2.out' });
      g.from('.s61-form-wrap', { x: 12, opacity: 0, duration: 0.4, delay: 0.08, ease: 'power2.out' });
    }
  });
}
