/**
 * 시안 66 - 실험적 인터페이스 + 카드 분리형 + GSAP 기반 부드러운 전환
 */
...
if (typeof document !== 'undefined') {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.s66-card').forEach(function (card) {
      var input = card.querySelector('.s66-input');
      if (!input || !g) return;
      input.addEventListener('focus', function () {
        g.to(card, { scale: 1.02, duration: 0.28, ease: 'power2.out' });
      });
      input.addEventListener('blur', function () {
        g.to(card, { scale: 1, duration: 0.28, ease: 'power2.out' });
      });
    });
  });
}
