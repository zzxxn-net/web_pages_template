/**
 * 시안 63 - 현대 판타지 + 단계 분할형 + 입력 반응형 애니메이션
 */
var EMAIL_REGEX = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
var MIN_PASSWORD_LENGTH = 8;
var STEP_FIELDS = ['email', 'name', 'password', 'passwordConfirm'];
...
if (typeof document !== 'undefined') {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.s63-input').forEach(function (input) {
      input.addEventListener('focus', function () {
        if (window.gsap) window.gsap.to(input, { scale: 1.02, duration: 0.22, ease: 'power2.out' });
      });
      input.addEventListener('blur', function () {
        if (window.gsap) window.gsap.to(input, { scale: 1, duration: 0.22, ease: 'power2.out' });
      });
    });
  });
}
