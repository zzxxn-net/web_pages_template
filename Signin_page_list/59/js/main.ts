...
if (typeof document !== 'undefined') {
  document.addEventListener('alpine:init', () => { window.Alpine?.data('signupForm', signupForm); });
  document.addEventListener('DOMContentLoaded', () => {
    const g = (window as unknown as { gsap?: { from: (el: string | Element, v: object) => void; to: (el: Element, v: object) => void } }).gsap;
    if (g) {
      g.from('.s59-full', { opacity: 0, y: 14, duration: 0.5, ease: 'power2.out' });
    }
    document.querySelectorAll('.s59-input').forEach((input) => {
      input.addEventListener('focus', () => { if (g) g.to(input, { scale: 1.02, duration: 0.24, ease: 'power2.out' }); });
      input.addEventListener('blur', () => { if (g) g.to(input, { scale: 1, duration: 0.24, ease: 'power2.out' }); });
    });
  });
}
