...
function signupForm78(): Record<string, unknown> {
  return {
    email: '', name: '', password: '', passwordConfirm: '',
    showPassword: false, submitting: false, focusedField: null as string | null, errors: {} as Record<string, string>,
    validateEmail(): void {
...
if (typeof document !== 'undefined') {
  document.addEventListener('alpine:init', () => { if (window.Alpine) window.Alpine.data('signupForm78', signupForm78); });
  document.addEventListener('DOMContentLoaded', () => {
    const g = (window as unknown as { gsap?: { fromTo: (a: string | Element, b: object, c: object) => void } }).gsap;
    if (g) { const v = document.querySelector('.s78-viewport'); if (v) g.fromTo(v, { opacity: 0, y: 8 }, { opacity: 1, y: 0, duration: 0.45, ease: 'power2.out' }); }
  });
}
export { signupForm78 };
