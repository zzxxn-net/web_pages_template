/**
 * 시안 87 - 사이버 미니멀 + 풀스크린 몰입형 + 스크롤 없는 고정 플로우
 * 단일 화면 폼, 유효성 검사, GSAP 로드 페이드인
 */
declare global {
  interface Window {
...
function signupForm87(): Record<string, unknown> {
  return {
    email: '',
    name: '',
    password: '',
...
if (typeof document !== 'undefined') {
  document.addEventListener('alpine:init', () => { if (window.Alpine) window.Alpine.data('signupForm87', signupForm87); });
  document.addEventListener('DOMContentLoaded', () => {
    const g = (window as unknown as { gsap?: { from: (t: string, v: object) => void } }).gsap;
    if (g) g.from('.s87-viewport', { opacity: 0, y: 14, duration: 0.45, ease: 'power2.out' });
  });
}
