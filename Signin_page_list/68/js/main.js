/**
 * 시안 68 - 사이버 미니멀 + 단일 포커스 입력 흐름 + 입력 반응형 애니메이션
 */
...
if (typeof document !== 'undefined') {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.s68-input').forEach(function (input) {
      if (!g) return;
      input.addEventListener('focus', function () { g.to(input, { scale: 1.02, duration: 0.24, ease: 'power2.out' }); });
      input.addEventListener('blur', function () { g.to(input, { scale: 1, duration: 0.24, ease: 'power2.out' }); });
    });
  });
}
