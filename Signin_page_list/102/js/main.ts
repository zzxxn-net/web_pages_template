...
function signupForm102(): Record<string, unknown> {
  return {
    email: '',
    name: '',
    password: '',
    ...
    goToAccountCard(): void {
      (this as { validateEmail: () => void }).validateEmail();
      (this as { validateName: () => void }).validateName();
      const e = (this as { errors: Record<string, string> }).errors;
      if (e.email || e.name) return;
      const refs = (this as { $refs: { cardAccount?: HTMLElement; inputPassword?: HTMLInputElement } }).$refs;
      const input = refs?.inputPassword;
      if (input) {
        refs?.cardAccount?.scrollIntoView?.({ behavior: 'smooth', block: 'center' });
        setTimeout(() => input.focus(), 300);
      }
    },

    goToBasicCard(): void {
      const refs = (this as { $refs: { cardBasic?: HTMLElement; inputEmail?: HTMLInputElement } }).$refs;
      const input = refs?.inputEmail;
      if (input) {
        refs?.cardBasic?.scrollIntoView?.({ behavior: 'smooth', block: 'center' });
        setTimeout(() => input.focus(), 300);
      }
    },
...
if (typeof document !== 'undefined') {
  document.addEventListener('alpine:init', () => { if (window.Alpine) window.Alpine.data('signupForm102', signupForm102); });
