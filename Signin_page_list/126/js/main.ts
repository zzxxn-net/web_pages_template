/** 시안 126: 사이버 미니멀 + 비대칭 레이아웃 + 스크롤 없는 고정 플로우 */
declare global {
  interface Window {
...
function signupForm126(): Record<string, unknown> {
  return {
    email: '',
    name: '',
    password: '',
    ...
    focusNext(nextId: string): void {
      const el = document.getElementById('s126-' + nextId);
...
if (typeof document !== 'undefined') {
  document.addEventListener('alpine:init', () => {
    if (window.Alpine) {
      window.Alpine.data('signupForm126', signupForm126);
    }
  });
  document.addEventListener('DOMContentLoaded', () => {
    const g = window.gsap;
    const inner = document.querySelector('.s126-inner');
    const header = document.querySelector('.s126-aside');
    const formWrap = document.querySelector('.s126-form-wrap');
    if (g && inner) {
