...
function runPanelIn81(stepNum: number): void {
  const g = (window as unknown as { gsap?: { fromTo: (t: HTMLElement, from: object, to: object) => void } }).gsap;
  const panels = document.querySelectorAll<HTMLElement>('.s81-panel');
...
function signupForm81(): Record<string, unknown> {
  return {
    ...
    goNext(): void {
      ...
      requestAnimationFrame(() => runPanelIn81(2));
    },

    goPrev(): void {
      (this as { step: number }).step = 1;
      requestAnimationFrame(() => runPanelIn81(1));
    },
...
if (typeof document !== 'undefined') {
  document.addEventListener('alpine:init', () => { if (window.Alpine) window.Alpine.data('signupForm81', signupForm81); });
