/** 시안 120: 현대 판타지 + 비대칭 레이아웃 + 스크롤 없는 고정 플로우 */
declare global {
  interface Window {
...
function signupForm120(): Record<string, unknown> {
  return {
    email: '',
    name: '',
    password: '',
    ...
    focusNext(nextId: string): void {
      const el = document.getElementById('s120-' + nextId);
...
if (typeof document !== 'undefined') {
  document.addEventListener('alpine:init', () => {
    if (window.Alpine) {
      window.Alpine.data('signupForm120', signupForm120);
    }
  });
  document.addEventListener('DOMContentLoaded', () => {
    const g = window.gsap;
    const inner = document.querySelector('.s120-inner');
    const header = document.querySelector('.s120-header');
    const formWrap = document.querySelector('.s120-form-wrap');
    if (g && inner) {
