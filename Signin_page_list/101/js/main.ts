/**
 * 시안 101 - 미래적 클린 + 풀스크린 몰입형 + 스크롤 없는 고정 플로우
 */
declare global {
  interface Window {
...
function signupForm101(): Record<string, unknown> {
  return {
    email: '',
    name: '',
    password: '',
...
if (typeof document !== 'undefined') {
  document.addEventListener('alpine:init', () => { if (window.Alpine) window.Alpine.data('signupForm101', signupForm101); });
  document.addEventListener('DOMContentLoaded', () => {
    const g = (window as unknown as { gsap?: { from: (t: string | HTMLElement, v: object) => void } }).gsap;
    if (g) g.from('.s101-layout', { opacity: 0, y: 8, duration: 0.32, ease: 'power2.out' });
  });
}

export { signupForm101 };
