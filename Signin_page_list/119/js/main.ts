/** 시안 119: 감성 테크 + 풀스크린 몰입형 + 입력 반응형 애니메이션 */
declare global {
  interface Window {
...
function signupForm119(): Record<string, unknown> {
  return {
    email: '',
    name: '',
    password: '',
    ...
    focusNext(nextId: string): void {
      const el = document.getElementById('s119-' + nextId);
...
if (typeof document !== 'undefined') {
  document.addEventListener('alpine:init', () => {
    if (window.Alpine) {
      window.Alpine.data('signupForm119', signupForm119);
    }
  });
  document.addEventListener('DOMContentLoaded', () => {
    const g = window.gsap;
    const inner = document.querySelector('.s119-inner');
    const header = document.querySelector('.s119-header');
    const formWrap = document.querySelector('.s119-form-wrap');
    if (g && inner) {
