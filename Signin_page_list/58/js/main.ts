/**
 * 시안 58 - 현대 판타지 + 단일 포커스 + GSAP 부드러운 전환
 */
declare global {
  interface Window {
...
if (typeof document !== 'undefined') {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (g) {
      g.from('.s58-wrap', { opacity: 0, y: 16, duration: 0.55, ease: 'power2.out' });
      g.from('.s58-field', { opacity: 0, y: 10, duration: 0.4, stagger: 0.07, delay: 0.1, ease: 'power2.out' });
      g.from('.s58-actions', { opacity: 0, y: 8, duration: 0.4, delay: 0.38, ease: 'power2.out' });
    }
  });
}
