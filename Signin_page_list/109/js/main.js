...
if (typeof document !== 'undefined') {
  document.addEventListener('alpine:init', function () {
    if (window.Alpine) window.Alpine.data('signupForm109', signupForm109);
  });
  document.addEventListener('DOMContentLoaded', function () {
    const g = window.gsap;
    const inner = document.querySelector('.s109-inner');
    const header = document.querySelector('.s109-header');
    const formWrap = document.querySelector('.s109-form-wrap');
    if (g && inner) {
