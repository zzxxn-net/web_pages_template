...
function runPanelIn86(stepNum: number): void {
  const g = (window as unknown as { gsap?: { fromTo: (t: HTMLElement, from: object, to: object) => void } }).gsap;
  const panels = document.querySelectorAll<HTMLElement>('.s86-panel');
...
function signupForm86(): Record<string, unknown> {
  return {
    step: 1,
    email: '',
    name: '',
    ...
    goNext(): void {
      ...
      requestAnimationFrame(() => runPanelIn86(2));
    },

    goPrev(): void {
      (this as { step: number }).step = 1;
      requestAnimationFrame(() => runPanelIn86(1));
    },
...
if (typeof document !== 'undefined') {
  document.addEventListener('alpine:init', () => { if (window.Alpine) window.Alpine.data('signupForm86', signupForm86); });
  document.addEventListener('DOMContentLoaded', () => {
    const g = (window as unknown as { gsap?: { from: (t: string, v: object) => void } }).gsap;
    if (g) g.from('.s86-wrap', { opacity: 0, y: 16, duration: 0.5, ease: 'power2.out' });
  });
}
