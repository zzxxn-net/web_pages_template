...
function signupForm125(): Record<string, unknown> {
  return {
    email: '',
    name: '',
    password: '',
    ...
    focusNext(nextId: string): void {
      const el = document.getElementById('s125-' + nextId);
...
if (typeof document !== 'undefined') {
  document.addEventListener('alpine:init', () => {
    if (window.Alpine) {
      window.Alpine.data('signupForm125', signupForm125);
    }
  });
  document.addEventListener('DOMContentLoaded', () => {
    const g = window.gsap;
    const inner = document.querySelector('.s125-inner');
    const header = document.querySelector('.s122-header');
    const formWrap = document.querySelector('.s125-form-wrap');
    if (g && inner) {
