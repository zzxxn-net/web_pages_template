/** 시안 125: 다크 아카이브 + 풀스크린 몰입형 + GSAP 기반 부드러운 전환 */
var EMAIL_REGEX = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
var MIN_PASSWORD_LENGTH = 8;

function signupForm125() {
  return {
    email: '',
    name: '',
    password: '',
    ...
    focusNext: function (nextId) {
      var el = document.getElementById('s125-' + nextId);
...
if (typeof document !== 'undefined') {
  document.addEventListener('alpine:init', function () {
    if (window.Alpine) window.Alpine.data('signupForm122', signupForm122);
  });
  document.addEventListener('DOMContentLoaded', function () {
    var g = window.gsap;
    var inner = document.querySelector('.s125-inner');
    var header = document.querySelector('.s125-header');
    var formWrap = document.querySelector('.s125-form-wrap');
    if (g && inner) {
