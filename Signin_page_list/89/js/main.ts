...
function signupForm89(): Record<string, unknown> {
  return {
    email: '',
    name: '',
    password: '',
...
if (typeof document !== 'undefined') {
  document.addEventListener('alpine:init', () => { if (window.Alpine) window.Alpine.data('signupForm89', signupForm89); });
  document.addEventListener('DOMContentLoaded', () => {
    const g = (window as unknown as { gsap?: { from: (t: string | HTMLElement, v: object) => void; to: (t: string | HTMLElement, v: object) => void } }).gsap;
    if (g) {
      g.from('.s89-layout', { opacity: 0, duration: 0.4, ease: 'power2.out' });
      g.from('.s89-brand', { opacity: 0, x: -24, duration: 0.55, ease: 'power2.out', delay: 0.05 });
      g.from('.s89-form-section', { opacity: 0, x: 24, duration: 0.55, ease: 'power2.out', delay: 0.1 });
    }
  });
}
