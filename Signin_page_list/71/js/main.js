...
if (typeof document !== 'undefined') {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    var g = window.gsap;
    if (!g) return;
    var box = document.querySelector('.s71-box');
    if (box) g.from(box, { opacity: 0, y: 16, duration: 0.4, ease: 'power2.out' });
    document.querySelectorAll('.s71-input').forEach(function (input) {
      input.addEventListener('focus', function () {
        g.to(input, { scale: 1.02, duration: 0.2, ease: 'power2.out' });
      });
      input.addEventListener('blur', function () {
        g.to(input, { scale: 1, duration: 0.2, ease: 'power2.out' });
      });
    });
  });
}
