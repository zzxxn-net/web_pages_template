/**
 * 회원가입 시안 26 - 단계 분할형(Progressive) + 다크 아카이브
 * GSAP 기반 부드러운 전환 · Alpine.js: step, 검증, 폼 제출
 */
...
function signupForm() {
  return {
    ...
    goNext(): void {
      ...
      this.step = 2;
      requestAnimationFrame(() => runPanelIn(2));
    },

    goPrev(): void {
      this.step = 1;
      requestAnimationFrame(() => runPanelIn(1));
    },
...
function runPanelIn(stepNum: number): void {
  ...
  const panels = document.querySelectorAll<HTMLElement>('.archstep-panel');
