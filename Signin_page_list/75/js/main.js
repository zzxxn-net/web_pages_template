...
if (typeof document !== 'undefined') {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (gsap) {
      var wrap = document.querySelector('.s75-wrap');
      var cards = document.querySelectorAll('.s75-card');
      if (wrap) {
        gsap.fromTo(wrap, { opacity: 0, y: 12 }, { opacity: 1, y: 0, duration: 0.5, ease: 'power2.out' });
      }
      if (cards.length) {
        gsap.fromTo(cards, { opacity: 0, y: 8 }, { opacity: 1, y: 0, duration: 0.4, stagger: 0.06, delay: 0.15, ease: 'power2.out' });
      }
    }
  });
}
