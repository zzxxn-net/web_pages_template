...
function signupForm() {
  return {
    ...
    focusInputForStep(s: number): void {
    ...
    goNext(): void {
      ...
      const g = (window as unknown as { gsap?: { to: (t: Element, v: object) => void; from: (t: Element, v: object) => void } }).gsap;
      const refs = (this as unknown as { $refs: Record<string, HTMLElement | undefined> }).$refs;
      const steps = [refs.step1, refs.step2, refs.step3, refs.step4].filter(Boolean) as HTMLElement[];
