...
function signupForm110(): Record<string, unknown> {
  return {
    email: '',
    name: '',
    password: '',
...
if (typeof document !== 'undefined') {
  document.addEventListener('alpine:init', () => { if (window.Alpine) window.Alpine.data('signupForm110', signupForm110); });
  document.addEventListener('DOMContentLoaded', () => {
    const g = (window as unknown as { gsap?: { from: (el: string | HTMLElement, v: object) => void } }).gsap;
    const layout = document.querySelector('.s110-layout');
    const cardBrand = document.querySelector('.s110-card--brand');
    const cardForm = document.querySelector('.s110-card--form');
    if (g && layout) {
