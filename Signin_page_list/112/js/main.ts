/** 시안 112: 다크 아카이브 + 마이크로 인터랙션 + 스크롤 없는 고정 플로우 */
declare global {
  interface Window {
    Alpine: { data: (name: string, fn: () => object) => void };
    gsap?: { from: (el: string | HTMLElement, v: object) => void; to: (el: string | HTMLElement, v: object) => void };
  }
}
const EMAIL_REGEX = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
const MIN_PASSWORD_LENGTH = 8;

function signupForm112(): Record<string, unknown> {
  return {
    email: '',
    name: '',
    password: '',
    ...
    onFocus(ev: Event): void {
      const el = (ev.target as HTMLElement).closest('.s112-field');
      if (window.gsap && el) {
        window.gsap.to(el, { scale: 1.002, duration: 0.2, ease: 'power2.out' });
      }
    },
    validateEmail(): void {
...
if (typeof document !== 'undefined') {
  document.addEventListener('alpine:init', () => { if (window.Alpine) window.Alpine.data('signupForm111', signupForm111); });
  document.addEventListener('DOMContentLoaded', () => {
    const g = (window as unknown as { gsap?: { from: (el: string | HTMLElement, v: object) => void } }).gsap;
    const inner = document.querySelector('.s112-inner');
    const header = document.querySelector('.s111-header');
    const formWrap = document.querySelector('.s112-form-wrap');
    if (g && inner) {
      g.from(inner, { opacity: 0, duration: 0.45, ease: 'power2.out' });
      if (header) g.from(header, { opacity: 0, y: -8, duration: 0.35, ease: 'power2.out', delay: 0.08 });
      if (formWrap) g.from(formWrap, { opacity: 0, y: 8, duration: 0.35, ease: 'power2.out', delay: 0.15 });
    }
  });
}
export { signupForm112 };
