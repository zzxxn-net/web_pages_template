...
if (typeof document !== 'undefined') {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    var g = window.gsap;
    if (!g) return;
    var layout = document.querySelector('.s69-layout');
    var aside = document.querySelector('.s69-aside');
    var formWrap = document.querySelector('.s69-form-wrap');
    ...
    document.querySelectorAll('.s69-input').forEach(function (input) {
      input.addEventListener('focus', function () {
        g.to(input, { scale: 1.01, duration: 0.28, ease: 'power2.out' });
      });
      input.addEventListener('blur', function () {
        g.to(input, { scale: 1, duration: 0.28, ease: 'power2.out' });
      });
    });
  });
}
