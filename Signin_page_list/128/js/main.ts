...
function signupForm128(): Record<string, unknown> {
  return {
    step: 1,
    email: '',
    name: '',
...
if (typeof document !== 'undefined') {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    const g = window.gsap;
    const inner = document.querySelector('.s128-inner');
    const header = document.querySelector('.s128-header');
    const formWrap = document.querySelector('.s128-form-wrap');
    if (g && inner) {
