...
function signupForm124(): Record<string, unknown> {
  return {
    email: '',
    name: '',
    password: '',
    ...
    onFocus(field: string): void {
      ...
      const activeEl = document.querySelector(`.s124-field[data-field="${field}"]`);
      const dimmed = document.querySelectorAll('.s124-field--dimmed');
      if (g && activeEl) {
        g.to(activeEl, { opacity: 1, scale: 1, duration: 0.3, ease: 'power2.out' });
        dimmed.forEach((el) => g.to(el, { opacity: 0.58, duration: 0.28, ease: 'power2.out' }));
      }
    },
    ...
    focusNext(nextId: string): void {
      const el = document.getElementById('s124-' + nextId);
...
if (typeof document !== 'undefined') {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    const g = window.gsap;
    const inner = document.querySelector('.s124-inner');
    const header = document.querySelector('.s124-header');
    const formWrap = document.querySelector('.s124-form-wrap');
    if (g && inner) {
