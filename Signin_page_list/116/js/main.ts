/** 시안 116: 사이버 미니멀 + 풀스크린 몰입형 + 포커스 이동 중심 UX */ declare global {
  interface Window {
...
function signupForm114(): Record<string, unknown> {
  return {
    ...
    focusNext(nextId: string): void {
      const el = document.getElementById('s116-' + nextId);
...
if (typeof document !== 'undefined') {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    const g = window.gsap;
    const inner = document.querySelector('.s116-inner');
    const header = document.querySelector('.s116-header');
    const formWrap = document.querySelector('.s116-form-wrap');
    if (g && inner) {
