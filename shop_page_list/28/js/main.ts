...
function shopPage28() {
  ...
  return {
    ...
    setFocus(id: number) {
      ...
      const inner = document.querySelector(".focus-28__inner");
      const prevCard = document.querySelector(".focus-card-28[data-product=\"" + this.focusId + "\"]");
      const nextCard = document.querySelector(".focus-card-28[data-product=\"" + id + "\"]");
      if (prevCard && nextCard && inner) {
    ...
    addToCart(id: number) {
      ...
      if (typeof gsap !== "undefined") {
        const btn = document.querySelector(".focus-card-28[data-product=\"" + id + "\"] .btn-28");
        if (btn) {
    ...
    setupFocusTransitions() {
      ...
      setTimeout(() => {
        const cards = document.querySelectorAll(".focus-card-28");
        cards.forEach((el) => {
          gsap.set(el, { opacity: 0, scale: 0.96 });
        });
        const active = document.querySelector(".focus-card-28--active");
        if (active) {
