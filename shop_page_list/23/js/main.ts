  ...
  return {
    ...
    addToCart(id: number) {
      ...
      if (typeof gsap !== "undefined") {
        const btn = document.querySelector(".card-23[data-product=\"" + id + "\"] .btn-23");
        if (btn) {
    ...
    setupInputReactive() {
      ...
      const run = () => {
        const cards = document.querySelectorAll(".card-23");
        cards.forEach((el) => {
        ...
        const btns = document.querySelectorAll(".btn-23");
        btns.forEach((el) => {
...
if (typeof Alpine !== "undefined") {
  Alpine.data("shopPage23", shopPage23);
}
