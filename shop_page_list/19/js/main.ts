// mvp
/**
 * 시안 19: 현대 판타지 + 마이크로 인터랙션 중심 + 입력 반응형 애니메이션
 * Alpine.js 장바구니 + GSAP 호버·클릭 마이크로 인터랙션
 */
  ...
  return {
    ...
    addToCart(id: number) {
      ...
      if (typeof gsap !== "undefined") {
        const btn = document.querySelector(".card-19[data-product=\"" + id + "\"] .btn-19");
        if (btn) {
    ...
    setupInputReactive() {
      ...
      const run = () => {
        const cards = document.querySelectorAll(".card-19");
        cards.forEach((el) => {
        ...
        const btns = document.querySelectorAll(".btn-19");
        btns.forEach((el) => {
...
if (typeof Alpine !== "undefined") {
  Alpine.data("shopPage19", shopPage19);
}
