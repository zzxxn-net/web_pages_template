  ...
  return {
    ...
    goStep(next: number) {
      ...
      if (typeof gsap !== "undefined") {
        ...
        if (next === 2) {
          setTimeout(() => {
            const first = document.querySelector(".card-20[data-product=\"1\"]") as HTMLElement | null;
            if (first) first.focus({ preventScroll: false });
          }, 100);
        }
        if (next === 3) {
          setTimeout(() => {
            const btn = document.querySelector(".shop-page-20__panel--cart .btn-20") as HTMLElement | null;
